...
\header{Colin}{LEVERGER}
      {Ingénieur Data \& DevOps, chercheur}
...
\begin{entrylist}
  \entry
    ...
    Développement d’algorithmes d’apprentissage automatique avec Python, Pandas, R, et analyse de séries temporelles pour faire des prévisions dans le contexte des performances des infrastructures.
    Localisation conjointe au laboratoire INRIA et à Orange Labs.
    Publication de 3 papiers de recherche dans des conférences nationales et internationales.% (détails et références disponibles sur demande).
    }
  \entry
    {02/19 - 05/19}
    {Chercheur}
    ...
    Développement d'un dashboard R Shiny pour afficher nombreuses time séries.
    Initier une toute nouvelle collaboration de recherche.
    }
       % \vspace{-1.1em}
  \entry
    {09/17 - 11/18}
    ...
    Créer et proposer un nouveau sujet de TP Python 3.% basé sur une application de scrapping en Python 3.
    }
  \entry
    {09/14 - 09/17}
    ...
    Expérimentation avec la stack ELK, Graphana et influxdb pour le traitement d’une masse importante de logs.
    Développement d’algorithmes d’apprentissage automatique pour le planning capacitaire.
    Utilisation de Scala, Spark, Elasticsearch, Logstash, Kibana, R, Openstack, etc.    
}
  \entry
    {02/15 - actuel}
    ...
    Utilisation de Docker pour gérer les services virtuels, Jenkins pour faire de l'intégration continue, Ansible pour scripter la configuration des serveurs. Expérimentations avec la stack ELK et Graphana pour monitorer les services et les attaques cyber.    
   }
%  \entry
%    {04/14 - 07/14}
%    {Internship}
